...
def _write_sample(root, name, object_names=("010", "002"), frames=49):
    ...
    for object_name in object_names:
        ...
        with h5py.File(object_dir / "pc.hdf5", "w") as source:
            ...
            source.create_dataset(
                "initial_angular_velocity", data=np.full((1, 3), -value, dtype=np.float32)
            )
    return sample
...
def test_joint_dataset_loads_video_and_lexically_sorted_objects(tmp_path):
    ...
    sample = JointWanPhysCtrlDataset(
        tmp_path, expected_size=(2, 2), expected_points=2
    )[0]
...
def test_joint_collate_keeps_unrestricted_metadata_and_adds_only_the_video_batch_axis(tmp_path):
    _write_sample(tmp_path, "sample_0001", object_names=("000",))
    sample = JointWanPhysCtrlDataset(tmp_path, expected_size=(2, 2), expected_points=2)[0]
...
def test_joint_dataset_rejects_bad_metadata_and_bad_trajectory_frames(tmp_path):
    ...
    with h5py.File(sample / "objects" / "002" / "pc.hdf5", "r+") as source:
        ...
        source.create_dataset("point_cloud", data=np.zeros((48, 1, 2, 3), dtype=np.float32))
    with pytest.raises(ValueError, match=r"point_cloud must have shape \(49, 1, 2, 3\)"):
        JointWanPhysCtrlDataset(tmp_path, expected_size=(2, 2), expected_points=2)
...
def test_joint_config_rejects_invalid_resume_and_checkpoint_metric_settings(tmp_path):
    ...
    with pytest.raises(ValueError, match="denoised_latent_mse_every_steps must be a positive integer"):
        load_joint_config(path, [])

    path.write_text(_valid_config().replace("checkpoints_total_limit: 2", "checkpoints_total_limit: 0"))
    with pytest.raises(ValueError, match="checkpoints_total_limit must be a positive integer"):
        load_joint_config(path, [])

    path.write_text(_valid_config().replace("resume_from_checkpoint: null", "resume_from_checkpoint: 123"))
    with pytest.raises(ValueError, match="resume_from_checkpoint must be null, 'latest', or a path string"):
        load_joint_config(path, [])
