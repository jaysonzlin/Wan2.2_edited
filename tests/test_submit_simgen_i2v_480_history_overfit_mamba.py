...
def test_pc_mamba_launcher_preflights_then_launches_eight_gpu_pretraining(
    tmp_path: Path,
) -> None:
    """The PC job must use the Mamba stack and preserve its training invocation."""
    script = PROJECT_DIR / "submit_pretrain_simgen_pc_8gpu_mamba.sh"
    (tmp_path / "configs/accelerate").mkdir(parents=True)
    (tmp_path / "configs/accelerate/h200_8gpu_2node.yaml").write_text("{}\n")
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
