...
def main(config=None):
    ...
    from train_pc import create_pc_noise_scheduler, load_pc_checkpoint_with_fallback, prune_pc_checkpoints
    ...
    for _epoch in range(step + 1, config["num_train_epochs"] + 1):
        for batch in loader:
            ...
            if accelerator.sync_gradients:
                step += 1
                accelerator.log({"train/loss": mse_loss(prediction, target).detach().item(), "train/learning_rate": lr_scheduler.get_last_lr()[0]}, step=step)
                ...
                if step >= config["max_train_steps"]: return
